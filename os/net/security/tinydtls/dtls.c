...
dtls_get_peer(const dtls_context_t *ctx, const session_t *session) {
  ...
  if(ctx && session) {
    p = ctx->peers;
//    printf("\n---------------------------\n");
//    printf("\nsession P information(which is stored in the dtls_context_t):\n");
//    dtls_session_log(&(p->session));
//    printf("\n---------------------------\n");
//    printf("\nsession information\n");
//    dtls_session_log(session);
//    printf("\n---------------------------\n");
    while(p) {
		...
		size_t buf_len_array[], size_t buf_array_len)
{
  ...
  if ((type == DTLS_CT_HANDSHAKE && buf_array[0][0] != DTLS_HT_HELLO_VERIFY_REQUEST) ||
    ...
    if (n) {
      ...
      n->retransmit_cnt = 0;
      n->timeout = 20 * DTLS_TICKS_PER_SECOND;
      n->peer = peer;
...
/*static void dtls_remove_peer(dtls_context_t *ctx, dtls_peer_t *peer)
{
    delete_peer(&ctx->peers, peer);
    dtls_debug_session("removed peer", &peer->session);
    dtls_free_peer(peer);
